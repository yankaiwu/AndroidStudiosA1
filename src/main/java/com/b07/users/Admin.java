...
public class Admin extends Employee implements Serializable{
	...
	public boolean promoteEmployee(Employee employee, Context context) throws SQLException,
			NullParameterException, InvalidRoleException, UserNotFoundException {
		int roleId = DatabaseSelectHelper.getUserRoleId(employee.getId(), context);
		if (DatabaseSelectHelper.getRoleName(roleId, context).equalsIgnoreCase("EMPLOYEE")) {
			DatabaseUpdateHelper.updateUserRole(roleId, employee.getId(), context);
			return true;
		}
		return false;
	}

	public void viewSalesHistory(Context context) throws NullParameterException,
